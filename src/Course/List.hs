...
-- def f(a), do: fn b -> (fn c -> [a, b, c] end) end

-- def g(a, b), do: fn c -> f(a, b, c) end
...
filter f (head :. tail) = filter f tail

-- filter f =
--  let f' y ys = if f y then y :. ys else ys
--  in foldRight f' Nil
...
processPayment :: PaymentSetting -> IO ()
processPayment setting = do
  case setting of
    CC -> return ()
    Manual -> return ()

class PaymentSettingClass s where
  process :: s -> IO ()
  refund :: s -> IO ()

data CCPaymentSetting = MkCC P.String

data ACHPaymentSetting = MkACH P.String

instance PaymentSettingClass ACHPaymentSetting where
  process setting = do return ()

processPayment' :: (PaymentSettingClass s) => s -> IO ()
processPayment' setting = do
  process setting
